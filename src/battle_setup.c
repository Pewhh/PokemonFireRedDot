...
<<<<<<< ours
<<<<<<< ours
static u8 GetExpertGymLeaderPartyCount(void)
{
    u8 trainerPartySize = gTrainers[gTrainerBattleOpponent_A].partySize;

<<<<<<< ours
    if (trainerPartySize > 5)
        trainerPartySize = 5;
=======
    if (trainerPartySize > ARRAY_COUNT(gSelectedOrderFromParty))
        trainerPartySize = ARRAY_COUNT(gSelectedOrderFromParty);
>>>>>>> theirs
    return trainerPartySize;
}

static u8 CountSelectedExpertGymMons(void)
{
    u8 i;
    u8 count = 0;

<<<<<<< ours
    for (i = 0; i < sExpertGymLeaderEntryCount && i < NELEMS(gSelectedOrderFromParty); i++)
=======
    for (i = 0; i < sExpertGymLeaderEntryCount && i < ARRAY_COUNT(gSelectedOrderFromParty); i++)
>>>>>>> theirs
    {
...
static void PrepareExpertGymLeaderParty(void)
{
    ...
    sExpertGymLeaderSelectedCount = CountSelectedExpertGymMons();
    if (sExpertGymLeaderSelectedCount == 0)
    {
        sExpertGymLeaderBattleActive = FALSE;
        return;
    }

    sExpertGymLeaderBattleActive = TRUE;
    sExpertGymLeaderOriginalPartyCount = gPlayerPartyCount;
    CpuCopy32(gPlayerParty, sExpertGymLeaderPartyBackup, sizeof(gPlayerParty));
    CpuFill32(0, gPlayerParty, sizeof(gPlayerParty));

    for (i = 0; i < sExpertGymLeaderSelectedCount; i++)
    {
        u8 slot = gSelectedOrderFromParty[i] - 1;

        sExpertGymLeaderOriginalSlots[i] = slot;
        gPlayerParty[i] = sExpertGymLeaderPartyBackup[slot];
    }
    gPlayerPartyCount = sExpertGymLeaderSelectedCount;
    CalculatePlayerPartyCount();
}

static void BeginExpertGymLeaderSelection(void)
{
    sExpertGymLeaderBattleActive = FALSE;
    sExpertGymLeaderSelectedCount = 0;
    sExpertGymLeaderEntryCount = GetExpertGymLeaderPartyCount();
    if (sExpertGymLeaderEntryCount == 0)
    {
        StartConfiguredTrainerBattle();
        return;
    }
    SetCustomMultiBattleMonCount(sExpertGymLeaderEntryCount);
    gMain.savedCallback = CB2_StartExpertGymLeaderBattle;
    InitChooseMonsForBattle(CHOOSE_MONS_FOR_EXPERT_GYM_BATTLE);
    ScriptContext_Stop();
}

static bool8 ShouldStartExpertGymLeaderSelection(void)
{
...
static void CB2_StartExpertGymLeaderBattle(void)
{
    PrepareExpertGymLeaderParty();
    sExpertGymLeaderEntryCount = 0;

    if (!sExpertGymLeaderBattleActive)
    {
        StartConfiguredTrainerBattle();
        return;
    }
...
void StartTrainerBattle(void)
{
    if (ShouldStartExpertGymLeaderSelection())
    {
        BeginExpertGymLeaderSelection();
        return;
    }
...
static void RestoreExpertGymLeaderParty(void)
{
    ...
    CpuCopy32(sExpertGymLeaderPartyBackup, restoredParty, sizeof(restoredParty));
    for (i = 0; i < sExpertGymLeaderSelectedCount && i < PARTY_SIZE; i++)
        restoredParty[sExpertGymLeaderOriginalSlots[i]] = gPlayerParty[i];

    CpuCopy32(restoredParty, gPlayerParty, sizeof(gPlayerParty));
    gPlayerPartyCount = sExpertGymLeaderOriginalPartyCount;
    CalculatePlayerPartyCount();
